...
void receive_temp(void const * argument);
...
int main(void)
{
  ...
  osThreadDef(recive_temp_tas, receive_temp, osPriorityIdle, 0, 128);
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  read_sensor_values(&temperature, &humidity);
	 	  	  	  	  sprintf(setbuffer, "temperature=%.2f\r\n",temperature);
	 	  	  	  	  HAL_UART_Transmit(&huart2, (uint8_t *)setbuffer, strlen(setbuffer),1000); // Transmit the formatted temperature
	 	  	  	  	  HAL_Delay(1000);
	 	  	  	      void receive_temp(void const * argument);
	 	  	  	  	  sprintf(setbuffer, "humidity=%.2f\r\n",humidity);
	 	  	  	  	  HAL_UART_Transmit(&huart2, (uint8_t *)setbuffer, strlen(setbuffer),1000);
	 	  	  	  	  HAL_Delay(1000);
    /* USER CODE BEGIN 3 */
  }
  /* USER CODE END 3 */
...
void MQTT_Init()
{
	...
	/*********CMD+MQTTCONCFG---->LED ********/
	//memset(&buffer[0],0x00,strlen(buffer));
	sprintf (&buffer[0], "CMD+MQTTCONCFG=3,harishkumarslm-wnfp2j,,,,,,,,,\r\n");
	HAL_UART_Transmit(&huart1, (uint8_t*)buffer, strlen(buffer), 1000);
	HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), 1000);
	//memset(&buffer[0],0x00,strlen(buffer));
	//HAL_Delay(500);
...
/* USER CODE BEGIN Header_receive_temp */
/**
* @brief Function implementing the receive_temp_ta thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_receive_temp */
void receive_temp(void const * argument)
{
  /* USER CODE BEGIN receive_temp */
  /* Infinite loop */
  for(;;)
  {
	  read_sensor_values(&temperature, &humidity);
  ...
  /* USER CODE END receive_temp */
}

/* USER CODE BEGIN Header_send_temp */
/**
* @brief Function implementing the send_temp2Handl thread.
* @param argument: Not used
* @retval None
*/
