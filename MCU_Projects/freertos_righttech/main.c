...
osThreadId receive_temp_taHandle;
...
void recieve_temp(void const * argument);
...
int main(void)
{
  ...
  /* definition and creation of receive_temp_ta */
  osThreadDef(receive_temp_ta, recieve_temp, osPriorityIdle, 0, 128);
  receive_temp_taHandle = osThreadCreate(osThread(receive_temp_ta), NULL);
...
void MQTT_Init()
{
	...
	/*********CMD+MQTTCONCFG ********/
	//memset(&buffer[0],0x00,strlen(buffer));
	sprintf (&buffer[0], "CMD+MQTTCONCFG=3,mqtt-harishkumarslm-x5yr7m,,,,,,,,,\r\n");
	HAL_UART_Transmit(&huart1, (uint8_t*)buffer, strlen(buffer), 1000);
	HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), 1000);
	//memset(&buffer[0],0x00,strlen(buffer));
	//HAL_Delay(500);
...
void mqtt_data_send()
{
		char buffer[] = "EVT+SUBDATA=base/relay/led1,0\r\n";
//	char buffer2[] = "EVT+SUBDATA=base/relay/led2,0\r\n";
...
/* USER CODE BEGIN Header_recieve_temp */
/**
* @brief Function implementing the receive_temp_ta thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_recieve_temp */
void recieve_temp(void const * argument)
{
  /* USER CODE BEGIN recieve_temp */
  /* Infinite loop */
  for(;;)
  {
    osDelay(1);
  }
  /* USER CODE END recieve_temp */
}
